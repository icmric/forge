...
class InfiniteCanvasController extends ChangeNotifier implements Graph {
  InfiniteCanvasController({List<InfiniteCanvasNode> nodes = const [], List<InfiniteCanvasEdge> edges = const [], bool snapMovementToGrid = false, bool snapResizeToGrid = false}) {
    if (nodes.isNotEmpty) {
  ...
  void editText(String text, InfiniteCanvasNode node) {
    node.label = text;
  }
