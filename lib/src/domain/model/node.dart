...
import 'package:forge/src/presentation/utils/helpers.dart';
...
class InfiniteCanvasNode<T> {
  ...
  void update({
    ...
    if (offset != null && setCurrentlyResizing == true) {
      this.offset = offset;
    } else if (offset != null && setCurrentlyResizing == null && allowMove && !currentlyResizing) {
      if (snapMovementToGrid == true && gridSize != null) {
        final snappedX = _getClosestSnapPosition(offset.dx, size?.width ?? this.size.width, gridSize.width);
        final snappedY = _getClosestSnapPosition(offset.dy, size?.height ?? this.size.height, gridSize.height);
    ...
    if (isEditing != null) this.isEditing = isEditing;
    if (textController != null) this.textController = textController;
  }
